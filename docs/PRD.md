...
Menu 键的导航行为采用**后退栈**模型：短按 Menu 键弹出栈顶，回到上一级。
...
导航栈示例：
    ...
    菜单界面 ──Menu短按──▶ 主界面      (栈深1 → 0，回到主界面)
```

| Menu 按键 | 行为 | 适用页面 | 实现逻辑 |
|-----------|------|---------|---------|
| **短按** | 返回上一级 | 所有非主界面页面 | `PopNavigationStack()` — 弹出栈顶 |

其中"上一级"的定义：
- 当前在**菜单界面** → 上一级是**主界面**
- 当前在**子界面**（音量调节 / 播放模式切换） → 上一级是**菜单界面**

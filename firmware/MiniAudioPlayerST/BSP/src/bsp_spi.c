                             ...
                             uint32_t timeout)
{
    ...
    return HAL_SPI_TransmitReceive(context->hspi, &tx_data, rx_data, 1, timeout);
}
...
{
    if (!BSP_SPI_IsValid(context) || (tx_buf == NULL) || (rx_buf == NULL) || (len == 0U))
    {
        return HAL_ERROR;
    }

    return HAL_SPI_TransmitReceive(context->hspi, (uint8_t *)tx_buf, rx_buf, len, timeout);
}
                             ...
                             uint32_t timeout)
{
    ...
    return HAL_SPI_Transmit(context->hspi, (uint8_t *)buf, len, timeout);
}
...
{
    if (!BSP_SPI_IsValid(context) || (context->hspi->hdmatx == NULL) || (buf == NULL) || (len == 0U))
    {
        return HAL_ERROR;
    }

    return HAL_SPI_Transmit_DMA(context->hspi, (uint8_t *)buf, len);
}

HAL_StatusTypeDef BSP_SPI_TxRx_DMA(const bsp_spi_context_t *context, const uint8_t *tx_buf,
                                   uint8_t *rx_buf, uint16_t len)
{
    if (!BSP_SPI_IsValid(context) || (context->hspi->hdmatx == NULL) ||
        (context->hspi->hdmarx == NULL) || (tx_buf == NULL) || (rx_buf == NULL) || (len == 0U))
    {
        return HAL_ERROR;
    }

    return HAL_SPI_TransmitReceive_DMA(context->hspi, (uint8_t *)tx_buf, rx_buf, len);
}

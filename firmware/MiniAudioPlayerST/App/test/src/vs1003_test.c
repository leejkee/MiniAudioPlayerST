...
{
    LOG_DEBUG("GPIO", "%s: XRESET=%u XCS=%u XDCS=%u DREQ=%u",
                     phase,
...
static void VS1003_Test_PrintRegister(const char *name, uint8_t address)
{
    ...
    if (status == BSP_VS1003_OK) {
        LOG_DEBUG("SCI", "%-6s (0x%02X) = 0x%04X",
                         name,
                         (unsigned int)address,
                         (unsigned int)value);
    } else {
        LOG_DEBUG("SCI", "%-6s (0x%02X) read failed: status=%d (%s)",
                         name,
                         (unsigned int)address,
                         (int)status,
...
static void VS1003_Test_DumpRegisters(void)
{
    ...
    LOG_DEBUG("Message", "       STATUS[6:4]=0x3 identifies VS1003; STATUS[1:0] may change at runtime.");
...
static bsp_vs1003_status_t VS1003_Test_InitializeDevice(void)
{
    ...
    if (status != BSP_VS1003_OK) {
        ...
        LOG_DEBUG("FAIL", "Hardware reset: status=%d (%s)",
                         (int)status,
                         VS1003_Test_StatusText(status));
        if (BSP_VS1003_IsReady() == 0U) {
            ...
            LOG_DEBUG("Message", "       Measure PA8/XRESET and PB10/DREQ; check VS1003 power, GND and crystal.");
        }
        return status;
    }
    ...
    if (status != BSP_VS1003_OK) {
        vs1003_test_ready = 0U;
        LOG_DEBUG("FAIL", "SCI initialization: status=%d (%s)",
                         (int)status,
                         VS1003_Test_StatusText(status));
        ...
        LOG_DEBUG("Message", "       DREQ reset check passed; inspect XCS, SCK, MOSI and MISO with a logic analyzer.");
        return status;
    }
...
static void VS1003_Test_PlaySine(void)
{
    ...
    if (status == BSP_VS1003_OK) {
        ...
        vs1003_test_ready = 0U;
        LOG_DEBUG("FAIL", "Sine test: status=%d (%s)",
                         (int)status,
                         VS1003_Test_StatusText(status));
        if (status == BSP_VS1003_ERR_VERIFY) {
            LOG_DEBUG("Message", "       Register verification failed during sine setup or post-test restore.");
            VS1003_Test_PrintVerifyFailure();
            VS1003_Test_DumpRegisters();
        }
    }

/**
  ...
  * @brief   主界面播放器联合上机测试
  * @note    OLED 用于目视检查，USART2 同时输出播放器元数据和预期图标尺寸。
  ******************************************************************************
  */
...
static const char *_StateName(player_state_t state)
{
    static const char *const names[] = {
        "UNINIT", "STOPPED", "PLAYING", "PAUSED", "ERROR"
    };
...
{
    return ((pressed & (1U << key)) != 0U)
        ? FONT_ICON_LARGE_VISUAL_SIZE
...
void MainPage_Test_Run(void)
{
    ...
    } else if ((uint32_t)(now - last_report_tick)
               >= MAIN_PAGE_TEST_REPORT_MS) {

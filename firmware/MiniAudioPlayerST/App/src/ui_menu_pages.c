...
static const menu_item_t menu_items[MENU_ITEM_COUNT] = {{volume_label, 4U, &Page_Volume},
...
static void _ShowChineseText(uint8_t x, uint8_t y, const uint16_t *text, uint8_t length)
{
    uint8_t index;
...
static void _RenderMenu(void)
{
    ...
    for (index = 0U; index < MENU_ITEM_COUNT; index++)
    {
        ...
        _ShowChineseText(16U, y, menu_items[index].label, menu_items[index].label_length);
    }
    BSP_SSD1315_Refresh();
}
...
static void _MenuKey(bsp_key_id_t key)
{
    switch (key)
    {
        case KEY_OK:
            ...
            break;
        case KEY_L:
            menu_selection = (menu_selection == 0U) ? MENU_ITEM_COUNT - 1U : menu_selection - 1U;
            _RenderMenu();
            break;
        case KEY_R:
            menu_selection++;
...
{
    return (mode == PLAYER_MODE_REPEAT_ONE) ? repeat_one_label : repeat_all_label;
}
...
static void _PlayModeKey(bsp_key_id_t key)
{
    switch (key)
    {
        case KEY_L:
        case KEY_R:
            selected_mode = (selected_mode == PLAYER_MODE_REPEAT_ALL) ? PLAYER_MODE_REPEAT_ONE
...
PageHandler_t Page_Menu = {.on_enter = _MenuEnter, .on_key = _MenuKey, .on_tick = NULL};

PageHandler_t Page_Volume = {.on_enter = _VolumeEnter, .on_key = _VolumeKey, .on_tick = NULL};

PageHandler_t Page_PlayMode = {.on_enter = _PlayModeEnter, .on_key = _PlayModeKey, .on_tick = NULL};

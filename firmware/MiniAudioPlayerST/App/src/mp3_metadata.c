...
static uint32_t MP3_ReadSyncSafe32(const uint8_t *data)
{
    ...
    return ((uint32_t)data[0] << 21) | ((uint32_t)data[1] << 14) | ((uint32_t)data[2] << 7) |
           data[3];
}

static uint8_t MP3_ReadAt(FIL *file, uint32_t offset, uint8_t *data, uint16_t length)
{
    UINT bytes_read = 0U;

    if ((f_lseek(file, offset) != FR_OK) || (f_read(file, data, length, &bytes_read) != FR_OK))
    {
        return 0U;
    }
...
static uint8_t MP3_ParseTlenText(const uint8_t *data, uint16_t length, uint32_t *duration_seconds)
{
    ...
    while (offset < length)
    {
        ...
        if ((encoding == 1U) || (encoding == 2U))
        {
            ...
            character = little_endian ? ((uint16_t)data[offset + 1U] << 8) | data[offset]
...
static uint8_t MP3_ReadId3v2(FIL *file, uint32_t file_size, uint32_t *tag_end,
                             uint32_t *duration_seconds)
{
    ...
    *tag_end          = 0U;
    *duration_seconds = 0U;
    if ((file_size < sizeof(header)) || (MP3_ReadAt(file, 0U, header, sizeof(header)) == 0U) ||
    ...
    if ((tag_data_size == UINT32_MAX) || (tag_data_size > file_size - sizeof(header)))
    {
        return 0U;
    }
    ...
    *tag_end   = frames_end;
    ...
    if ((flags & 0x40U) != 0U)
    {
        ...
        extended_size = (version == 4U) ? MP3_ReadSyncSafe32(frame_header)
    ...
    {
        if ((MP3_ReadAt(file, frame_offset, frame_header, sizeof(frame_header)) == 0U) ||
            (frame_header[0] == 0U) || (MP3_IsFrameId(frame_header) == 0U))
        {
            break;
        }

        frame_size = (version == 4U) ? MP3_ReadSyncSafe32(&frame_header[4])
        ...
        if ((frame_header[0] == 'T') && (frame_header[1] == 'L') && (frame_header[2] == 'E') &&
            (frame_header[3] == 'N') && (frame_header[9] == 0U))
        {
            text_size = (frame_size > sizeof(text)) ? sizeof(text) : (uint16_t)frame_size;
            if ((text_size > 0U) &&
                (MP3_ReadAt(file, frame_offset + sizeof(frame_header), text, text_size) != 0U) &&
                (MP3_ParseTlenText(text, text_size, duration_seconds) != 0U))
            {
                return 1U;
            }
        }
...
static uint8_t MP3_ParseFrameHeader(const uint8_t *data, mp3_frame_info_t *frame)
{
    static const uint16_t bitrate_mpeg1_l3[16] = {0U,   32U,  40U,  48U,  56U,  64U,  80U,  96U,
                                                  112U, 128U, 160U, 192U, 224U, 256U, 320U, 0U};
    static const uint16_t bitrate_mpeg2_l3[16] = {0U,  8U,  16U, 24U,  32U,  40U,  48U,  56U,
                                                  64U, 80U, 96U, 112U, 128U, 144U, 160U, 0U};
    static const uint32_t sample_rates[3][3]   = {
        {11025U, 12000U, 8000U}, {22050U, 24000U, 16000U}, {44100U, 48000U, 32000U}};
    ...
    if (((header & 0xFFE00000UL) != 0xFFE00000UL) || (((header >> 17) & 0x03U) != 0x01U))
    {
        return 0U;
    }
    ...
    if ((version_id == 1U) || (bitrate_index == 0U) || (bitrate_index == 15U) ||
    ...
    frame->frame_size = (uint16_t)((coefficient * bitrate_kbps) / frame->sample_rate_hz + padding);
    frame->version_id = version_id;
    return (frame->frame_size >= MP3_FRAME_HEADER_SIZE) ? 1U : 0U;
}

static uint8_t MP3_ReadFrameAt(FIL *file, uint32_t offset, mp3_frame_info_t *frame)
{
...
static uint8_t MP3_FindFirstFrame(FIL *file, uint32_t search_start, uint32_t audio_end,
                                  uint8_t *scratch, uint16_t scratch_size, uint32_t *frame_offset,
                                  mp3_frame_info_t *first_frame, uint8_t *is_cbr)
{
    ...
    if ((scratch == NULL) || (scratch_size < MP3_FRAME_HEADER_SIZE) || (search_start >= audio_end))
    {
        return 0U;
    }
    ...
    {
        chunk_size = (search_end - offset > scratch_size) ? scratch_size
        ...
        for (index = 0U; index + MP3_FRAME_HEADER_SIZE <= bytes_read; index++)
        {
            ...
            if ((next_offset >= audio_end) || (MP3_ReadFrameAt(file, next_offset, &next) == 0U) ||
...
static uint32_t MP3_FindAudioEnd(FIL *file, uint32_t file_size)
{
    ...
    if ((file_size >= MP3_ID3V1_TAG_SIZE) &&
        (MP3_ReadAt(file, file_size - MP3_ID3V1_TAG_SIZE, signature, sizeof(signature)) != 0U) &&
        (signature[0] == 'T') && (signature[1] == 'A') && (signature[2] == 'G'))
    {
                      ...
                      mp3_metadata_t *metadata)
{
    ...
    if (MP3_FindFirstFrame(file, id3_end, audio_end, scratch, scratch_size, &frame_offset,
                           &first_frame, &is_cbr) != 0U)
    {
        ...
        else if ((is_cbr != 0U) && (first_frame.bitrate_bps != 0U))
        {
            metadata->duration_seconds =
                (uint32_t)(((uint64_t)metadata->audio_size * 8U) / first_frame.bitrate_bps);
        }
        return;
    }

    metadata->audio_offset     = id3_end;
    metadata->audio_size       = (audio_end > id3_end) ? audio_end - id3_end : 0U;
    metadata->duration_seconds = id3_duration;
}

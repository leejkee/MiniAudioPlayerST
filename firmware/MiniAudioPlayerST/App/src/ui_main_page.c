...
#define MAIN_CHANGED_ALL                                                                           \
    (MAIN_CHANGED_HEADER | MAIN_CHANGED_NAME | MAIN_CHANGED_CONTROLS | MAIN_CHANGED_ELAPSED |      \
...
static void _CopyDisplayName(WCHAR *destination, uint16_t capacity, const WCHAR *source)
{
    ...
    if ((length >= 4U) && (destination[length - 4U] == (WCHAR)'.') &&
...
{
    static const WCHAR no_music[] = {'N', 'o', ' ', 'M', 'u', 's', 'i', 'c', 0};
    ...
    _CopyDisplayName(main_meta.name, (uint16_t)(sizeof(main_meta.name) / sizeof(main_meta.name[0])),
                     name);
}
...
static void _ShowWideString(uint8_t x, uint8_t y, const WCHAR *text)
{
    while ((text != NULL) && (*text != 0U) && (x < OLED_WIDTH))
    {
        ...
        else
        {
            ...
            BSP_SSD1315_ShowChinese(x, y, (uint16_t)*text, FONT_TYPE_FILE);
...
static void _ShowSystemText(uint8_t x, uint8_t y, const uint16_t *text, uint8_t count)
{
    uint8_t index;
...
static void _ShowTime(uint8_t x, uint32_t seconds)
{
    ...
    minutes = seconds / 60U;
    BSP_SSD1315_ShowNum(x + 8U, 3U * MAIN_ROW_HEIGHT, minutes, 2U, 1U);
    BSP_SSD1315_ShowChar(x + 24U, 3U * MAIN_ROW_HEIGHT, ':', 1U);
    BSP_SSD1315_ShowNum(x + 32U, 3U * MAIN_ROW_HEIGHT, seconds % 60U, 2U, 1U);
...
static void _RenderHeader(void)
{
    ...
    BSP_SSD1315_ShowChar((uint8_t)(40U + digits * FONT_EN_CHAR_W), 0U, '%', 1U);
...
{
    return ((main_meta.pressed_keys & (1U << key)) != 0U) ? 1U : 0U;
}
...
static void _RenderControls(void)
{
    ...
    BSP_SSD1315_ShowIcon(MAIN_ICON_PREVIOUS_X, 2U * MAIN_ROW_HEIGHT, MAIN_ICON_PREVIOUS,
                         _IconLarge(KEY_L));
    BSP_SSD1315_ShowIcon(MAIN_ICON_ACTION_X, 2U * MAIN_ROW_HEIGHT, action_icon, _IconLarge(KEY_OK));
    BSP_SSD1315_ShowIcon(MAIN_ICON_NEXT_X, 2U * MAIN_ROW_HEIGHT, MAIN_ICON_NEXT, _IconLarge(KEY_R));
}
...
static void _Render(uint8_t changed)
{
    ...
    if ((changed & MAIN_CHANGED_NAME) != 0U)
    {
        if (changed != MAIN_CHANGED_ALL)
        {
            BSP_SSD1315_ClearArea(0U, MAIN_ROW_HEIGHT, OLED_WIDTH, MAIN_ROW_HEIGHT);
        }
    ...
    if ((changed & MAIN_CHANGED_CONTROLS) != 0U)
    {
        if (changed != MAIN_CHANGED_ALL)
        {
            BSP_SSD1315_ClearArea(0U, 2U * MAIN_ROW_HEIGHT, OLED_WIDTH, MAIN_ROW_HEIGHT);
        }
        _RenderControls();
    }
    if ((changed & MAIN_CHANGED_ELAPSED) != 0U)
    {
        if (changed != MAIN_CHANGED_ALL)
        {
            BSP_SSD1315_ClearArea(0U, 3U * MAIN_ROW_HEIGHT, 56U, MAIN_ROW_HEIGHT);
        }
    ...
    if ((changed & MAIN_CHANGED_DURATION) != 0U)
    {
        if (changed != MAIN_CHANGED_ALL)
        {
            BSP_SSD1315_ClearArea(64U, 3U * MAIN_ROW_HEIGHT, 56U, MAIN_ROW_HEIGHT);
        }
...
static uint8_t _UpdateMeta(uint8_t force)
{
    ...
    else
    {
        ...
        if ((main_meta.state != state) || (main_meta.pressed_keys != pressed_keys))
        {
...
static void _Page_Tick(void)
{
    ...
    if ((_ReadPressedKeys() != main_meta.pressed_keys) ||
        ((uint32_t)(now - last_refresh_tick) >= MAIN_REFRESH_INTERVAL_MS))
    {
        _Refresh(0U);
    }
}

PageHandler_t Page_Main = {.on_enter = _Page_Enter, .on_key = _Page_Key, .on_tick = _Page_Tick};
